...
client
	...
	// max 200 – https://developer.twitter.com/en/docs/twitter-api/v1/accounts-and-users/follow-search-get-users/api-reference/get-followers-list
  	.get("followers/list", { count: 200 }) 
  	.then(results => {

		let _ids = [];
		let _users = [];
		let _dont_tweet_until = "2020-03-31";		// data for cleaning account
		let _delete_flag = false;					// debug flag
			...
			if(val.status){
				...
				if( new Date(val.status.created_at) <= new Date(_dont_tweet_until) ){
					...
					if( _delete_flag ) {
						...
						console.log('DEBUG ACTIVE – Following should be removed if exists: ', val.screen_name)
					}
